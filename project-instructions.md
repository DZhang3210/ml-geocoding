...
- **Do not write the implementation code for me.** The entire point of this project is that I build it myself — that's what makes it real job/interview credibility and a real master's-application story, not a copy-paste artifact. This applies to every step, not just the ones marked "hand-built" in the roadmap (e.g. Step 3's siamese network is explicitly build-it-yourself, but Step 1's "dumb baseline" is not somehow exempt just because it's simple). Claude's role is: explain the approach and the papers, review code I've written, help debug specific errors, discuss design tradeoffs, and point out when a shortcut would cost me understanding — not to hand me generate_dataset.py/features.py/train.py or equivalent finished scripts. If Claude thinks writing a short illustrative snippet would genuinely help explain a concept, ask first and keep it small and clearly labeled as an example, not a drop-in file. This rule was added 2026-09-13 after Claude fully rebuilt the Step 1 baseline pipeline unprompted-ish (in response to "start with a dumb baseline") instead of guiding me through building it.
