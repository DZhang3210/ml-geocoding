...
- **Status note (2026-09-15):** Street-name-side address-component parsing now essentially complete — `Directional`, `is_extensional` (`EXT`), and `street_extension` (real street-type suffix) all cleanly extracted and stripped from a base `StreetName`, each verified against the full dataset rather than assumed. Added a stable `id` column. Fixed a real ZIP leading-zero data bug (`pd.read_csv` dtype inference, not the source data). House-number component confirmed to need no parsing (already clean integers). Started positive-pair generation: sourced official USPS Pub 28 abbreviation tables and built a working abbreviation-substitution corruption function (`corrupt_rows` in the notebook) with per-field independent trigger probabilities, producing corrupted rows that reference their source row via `original_id`. Still not started: character-level replacement corruption (design discussed, not built), hard-negative pair generation, the actual labeled pairs table (join on `original_id`), and the location-based train/val/test split. Corruption logic intentionally still lives in the notebook, not `corrupt_data.py` — promotion to a module was deliberately deferred until the parsing/corruption logic stabilizes further.
